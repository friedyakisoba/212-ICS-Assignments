...
Hans Cacalda
November 5, 2024
Purpose :Simulates shuffling a deck of cards
         using structures and typedef
         by initializing, shuffling, and displaying the card deck

modifications on lines: 28, 75-83, 120, 123
*/
...
struct card
{
    ...
    // added pointer color declaration
    char *color;
};
...
void initialize(Card deck[])
{
    ...
    for (i = 0; i < MAX_CARDS; i++)
    {
        ...
        // added if statement to add colors based on suiy
        if (i / MAX_RANKS == 0 || i / MAX_RANKS == 3)
        {
            //clubs and spades
            deck[i].color = "Black"; 
        }
...
/*    had to comment this out since it was printing in columns of 3 on my system
